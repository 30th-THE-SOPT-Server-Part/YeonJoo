...
const getMoviesBySearch = async (
    ...
    try {
        ...
        if (option === "title") {
            movies = await Movie.find({ title: { $regex: findRegex } })
                .sort({ createdAt: -1 }) //최신순 정렬
                .skip(perPage * (page - 1))
                .limit(perPage);
        } else if (option === "director") {
            movies = await Movie.find({ director: { $regex: findRegex } })
                .sort({ createdAt: -1 }) //최신순 정렬
                .skip(perPage * (page - 1))
                .limit(perPage);
        } else {
        ...
        const totalPage: number = await Movie.countDocuments({});
        const lastPage: number = Math.ceil(totalPage / perPage);
        const data = { movies, lastPage };
