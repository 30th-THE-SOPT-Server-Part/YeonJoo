...
const createUser = async (req: Request, res: Response) => {
  ...
  const userCreateDto: UserCreateDto = req.body; // UserCreateDto 로 req.body 받아오기
  ...
  try {
    ...
    if (!result) {
      return res
        ...
        .send(util.fail(statusCode.CONFLICT, message.EMAIL_DUPLICATED));
    }
...
const updateUser = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res
      ...
      .send(util.success(statusCode.NO_CONTENT, message.UPDATE_USER_SUCCESS));
  } catch (error) {
    console.log(error);
    //서버 내부에서 오류 발생
    res
