...
const createMovieComment = async (req: Request, res: Response) => {
    ...
    try {
        console.log(movieCommentCreateDto);
...
const updateMovieComment = async (req: Request, res: Response) => {
    ...
    try {
        const data = await MovieService.updateMovieComment(
            movieId,
            commentId,
            req.body.user.id, // JWT token payload
            commentUpdateDto
        );
        ...
        res.status(statusCode.OK).send(
            util.success(statusCode.OK, message.UPDATE_MOVIE_SUCCESS, data)
        );
    } catch (error) {
        console.log(error);
        //서버 내부에서 오류 발생
